...
+ (NSInteger )passcodeLength {
	return [[self sharedUser] _passcodeLength];
}
...
- (NSInteger)_lenghtPasscode {
	if (!_usesKeychain &&
		[self.delegate respondsToSelector:@selector(passcode)]) {
		
		NSString *tmp = [self.delegate passcode];
		return [tmp length];
	}
	
	NSString *tmp =  [LTHKeychainUtils getPasswordForUsername:_keychainPasscodeUsername
...
- (BOOL)_simpleLength {
	NSInteger tmp = PASSLEN;
//	NSInteger tmp = [self _passcodeLength];
	
	return (tmp == 4) && _isSimple;
}
...
- (void)clearCurrentlyOnScreenFlag {
	_isCurrentlyOnScreen = NO;
}
...
- (void)_setupDigitFields {
	...
	if (!_isFourDigits) {
		_fithDigitTextField = [self _makeDigitField];
		[_animatingView addSubview:_fithDigitTextField];
		
		_sixthDigitTextField = [self _makeDigitField];
		[_animatingView addSubview:_sixthDigitTextField];
	}
}
...
- (void)updateViewConstraints {
	...
	if (!_isFourDigits) {
		_fithDigitTextField.hidden = !self.isSimple;
		_sixthDigitTextField.hidden = !self.isSimple;
	}
		...
		else
		{
			...
			[NSLayoutConstraint constraintWithItem: _fourthDigitTextField
										...
										multiplier: 1.0f
										  constant: _horizontalGap * 1/6 - 2.0f];
			//                                      constant: _horizontalGap - 2.0f];
			NSLayoutConstraint *fithDigitX =
			[NSLayoutConstraint constraintWithItem: _fithDigitTextField
										 attribute: NSLayoutAttributeLeft
										 relatedBy: NSLayoutRelationEqual
											toItem: _animatingView
										 attribute: NSLayoutAttributeCenterX
										multiplier: 1.0f
										  constant: _horizontalGap - 2.0f];
			NSLayoutConstraint *sixthDigitX =
			[NSLayoutConstraint constraintWithItem: _sixthDigitTextField
										 attribute: NSLayoutAttributeLeft
										 relatedBy: NSLayoutRelationEqual
											toItem: _animatingView
										 attribute: NSLayoutAttributeCenterX
										multiplier: 1.0f
										  constant: _horizontalGap * 1.8 - 2.0f];
			NSLayoutConstraint *firstDigitY =
			[NSLayoutConstraint constraintWithItem: _firstDigitTextField
										 attribute: NSLayoutAttributeCenterY
										 relatedBy: NSLayoutRelationEqual
			...
			[NSLayoutConstraint constraintWithItem: _fithDigitTextField
										...
										multiplier: 1.0f
										  constant: _verticalGap];
			NSLayoutConstraint *sixthDigitY =
			[NSLayoutConstraint constraintWithItem: _sixthDigitTextField
										 attribute: NSLayoutAttributeCenterY
										 relatedBy: NSLayoutRelationEqual
											toItem: _enterPasscodeLabel
			...
			[self.view addConstraint:thirdDigitX];
			[self.view addConstraint:fourthDigitX];
			[self.view addConstraint:fithDigitX];
			[self.view addConstraint:sixthDigitX];
			[self.view addConstraint:firstDigitY];
			[self.view addConstraint:secondDigitY];
			[self.view addConstraint:thirdDigitY];
			[self.view addConstraint:fourthDigitY];
			[self.view addConstraint:fithDigitY];
			[self.view addConstraint:sixthDigitY];
		}
    }
    else {
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if (self.isSimple) {
		...
		{
			if (typedString.length >= 5) _fithDigitTextField.secureTextEntry = YES;
			else _fithDigitTextField.secureTextEntry = NO;
			if (typedString.length >= 6) _sixthDigitTextField.secureTextEntry = YES;
			else _sixthDigitTextField.secureTextEntry = NO;
			
			if (typedString.length == 6) {
			...
			if (typedString.length > 6) return NO;
		}
    }
...
- (void)_denyAccess {
	...
	if (!_isFourDigits) {
		[_fithDigitTextField.layer addAnimation: animation forKey: @"shake"];
		[_sixthDigitTextField.layer addAnimation: animation forKey: @"shake"];
	}
...
- (void)_resetTextFields {
	...
	if (!_isFourDigits) {
		_fithDigitTextField.secureTextEntry = NO;
		_sixthDigitTextField.secureTextEntry = NO;
	}
}
...
- (void)setIsSimple:(BOOL)isSimple inViewController:(UIViewController *)viewController asModal:(BOOL)isModal{
    if (!_isUserSwitchingBetweenPasscodeModes &&
        ...
        _isUserSwitchingBetweenPasscodeModes = NO;
        _isSimple = isSimple;
		_isFourDigits = [self _simpleLength];
        [self.view setNeedsUpdateConstraints];
    }
}
